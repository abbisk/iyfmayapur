...
const required = ["email", "amount", "mobile", "first_name", "last_name", "address_1", "pin_code", "district", "city", "state", "country"];
...
export default function handler(request, response) {
  ...
  try {
    ...
    if (missing.length || !Number.isFinite(amount) || amount <= 0) return response.status(400).json({ error: "Please complete all required donation details.", fields: missing });
    ...
    return response.status(200).json({ reference_id: referenceId, payment_url: `${GATEWAY_URL}?dept_code=${DEPARTMENT_CODE}&data=${encodeURIComponent(data)}` });
  } catch (error) {
    console.error(error);
